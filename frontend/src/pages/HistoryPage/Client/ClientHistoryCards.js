...
function DisplayClientHistory() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:3001/getDeliveryHistory/client/${user._profile.data.email}`).then((response) => {
      setOfferList(response.data);
    });
  }, [user._profile.data.email]);

  const toUpdateStatus = (_id) => {
    navigate("/client/ClientHistoryInfo", { state: { _id } });
  };
...
export default DisplayClientHistory;